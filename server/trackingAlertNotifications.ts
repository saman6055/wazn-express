...
export async function checkAndNotifyTrackingAlerts(): Promise<void> {
  try {
    ...
    if (alertingPackages.length === 0) {
      console.log("[Tracking Alerts] No packages requiring notifications");
      return;
    }
    ...
    if (success) {
      console.log(`[Tracking Alerts] Notification sent for ${alertingPackages.length} packages`);
    } else {
      console.warn("[Tracking Alerts] Failed to send notification");
    }
  } catch (error) {
    console.error("[Tracking Alerts] Error checking and notifying:", error);
  }
}
