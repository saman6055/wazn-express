...
import { notificationLogs, customers, packages, batches } from "../drizzle/schema";
...
async function sendEmail(to: string, subject: string, body: string): Promise<boolean> {
  if (!resend) {
    console.error("[Notification] Resend API key not configured. Set RESEND_API_KEY environment variable.");
    return false;
  }
  ...
  try {
    ...
    if (error) {
      console.error("[Notification] Resend email failed:", error);
      return false;
    }
...
export async function sendNotification(params: {
  ...
  if (!customer) {
    console.error(`[Notification] Customer ${customerId} not found`);
    return {};
  }

  const template = notificationTemplates[eventType];
  if (!template) {
    console.error(`[Notification] Template not found for ${eventType}`);
    return {};
  }
...
// Helper function to send purchase request notification
