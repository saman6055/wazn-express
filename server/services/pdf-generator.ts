  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const company = data.company || {
        name: COMPANY_INFO.name, nameKu: "وەزن ئێکسپرێس",
        address: COMPANY_INFO.address, addressKu: "",
        phone: COMPANY_INFO.phone, email: COMPANY_INFO.email,
        website: COMPANY_INFO.website, primaryColor: COLORS.primary,
      };
      ...
      doc.fillColor(COLORS.gray)
        .fontSize(9)
        .text(company.address, 50, 85)
        .text(`Phone: ${company.phone}`, 50, 98)
        .text(`Email: ${company.email}`, 50, 111);
      ...
      if (data.customerBalance !== undefined) {
        const balY = (data.notes ? (totalBoxY + (data.tax ? 40 : 25) + 100) : totalBoxY + 60);
        const bal = data.customerBalance;
        const isDebt = bal > 0;
        const isCredit = bal < 0;

        doc.rect(50, balY, 495, 40).fillColor(COLORS.lightGray).fill();
        doc.fillColor(COLORS.dark).fontSize(10)
          .text("Customer Balance:", 60, balY + 8);
        doc.fillColor(COLORS.gray).fontSize(9)
          .text(`${data.customer.name} - ${data.customer.code}`, 60, balY + 22);
        doc.fillColor(isDebt ? COLORS.danger : isCredit ? COLORS.success : COLORS.gray)
          .fontSize(14)
          .text(`$${Math.abs(bal).toFixed(2)}`, 400, balY + 10, { align: "right" });
        doc.fontSize(9)
          .text(isDebt ? "Owes" : isCredit ? "Credit" : "Settled", 400, balY + 26, { align: "right" });
      }
      ...
      doc.fillColor(COLORS.gray)
        ...
        .text(`${company.name} - ${company.website}`, 50, 735, { align: "center" });
