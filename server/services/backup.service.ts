...
async function createDatabaseBackup(options: BackupOptions) {
  ...
  try {
    appLogger.info("[Backup] Starting database backup...");
    ...
    try {
      ...
      appLogger.error("[Backup] Failed to send notification", { error: notifError instanceof Error ? notifError.message : String(notifError) });
    }

    return { id: backupId, filename, fileUrl, fileSize, backupContent: options.backupContent, recordsCount: exportResult.totalRecords };
  } catch (error) {
    appLogger.error("[Backup] Error", { error: error instanceof Error ? error.message : String(error) });
...
async function createFullBackup(options: BackupOptions) {
  ...
  try {
    ...
    appLogger.info("[Full Backup] Step 1: Creating database backup...");
    const dbBackup = await createDatabaseBackup({
      ...options,
      backupContent: "database_only",
    });
    appLogger.info("[Full Backup] Database backup completed", { filename: dbBackup.filename });

    // Step 2: Create S3 files backup
    appLogger.info("[Full Backup] Step 2: Creating S3 files backup...");
    const { backupS3Files } = await import("./s3Backup.service");
    const s3Result = await backupS3Files();
    appLogger.info("[Full Backup] S3 backup completed", { fileCount: s3Result.fileCount });
    ...
    appLogger.error("[Full Backup] Error", { error: error instanceof Error ? error.message : String(error) });
    await db.update(backups)
      .set({
        status: "failed",
        errorMessage: error instanceof Error ? error.message : String(error),
...
export async function restoreBackup(backupId: number) {
  ...
  appLogger.info("[Restore] Starting restore from", { filename: backup.filename });
  ...
  try {
    ...
    appLogger.info("[Restore] Backup version", { version: backupData.version || "1.0" });
    appLogger.info("[Restore] Total records", { totalRecords: backupData.totalRecords || "unknown" });

    // Restore all tables using importAllData
    const tables = backupData.tables;
    
    appLogger.info("[Restore] Starting restore of tables", { tableCount: Object.keys(tables).length });
    ...
    for (const [category, result] of Object.entries(importResult.categoryResults)) {
      ...
      appLogger.info("[Restore] category imported/skipped", { category, imported: result.imported, skipped: result.skipped });
    }
    ...
    if (backup.backupContent === "full" && backup.filesZipUrl) {
      try {
        appLogger.info("[Restore] Restoring S3 files from", { filesZipUrl: backup.filesZipUrl });
        const { restoreS3Files } = await import("./s3Restore.service");
        filesRestored = await restoreS3Files(backup.filesZipUrl);
        appLogger.info("[Restore] Restored S3 files", { filesRestored });
      } catch (s3Error) {
        appLogger.error("[Restore] Failed to restore S3 files", { error: s3Error instanceof Error ? s3Error.message : String(s3Error) });
    ...
    appLogger.error("[Restore] Error", { error: error instanceof Error ? error.message : String(error) });
    throw error;
  }
}
