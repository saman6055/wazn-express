...
Be thorough and accurate. If text is unclear, indicate uncertainty.`
    ...
    appLogger.error("OCR Error", { error: error instanceof Error ? error.message : String(error) });
...
Be accurate and conservative with damage assessment. If unsure, lean toward "intact".`
    ...
    appLogger.error("Vision AI Error", { error: error instanceof Error ? error.message : String(error) });
    return { success: false, error: String(error) };
  }
}
...
export async function translateText(
  ...
  try {
    ...
    appLogger.error("Translation Error", { error: error instanceof Error ? error.message : String(error) });
    return { success: false, error: String(error) };
  }
}
...
Be thorough and accurate. Return structured JSON.`
    ...
    appLogger.error("Package Info Extraction Error", { error: error instanceof Error ? error.message : String(error) });
    return { success: false, error: String(error) };
  }
}
...
- Extract the code AND any name that follows it`
    ...
    appLogger.error("Package Label Scan Error", { error: error instanceof Error ? error.message : String(error) });
    return { success: false, error: String(error) };
  }
}
