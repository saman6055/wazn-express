...
export async function restoreS3Files(filesZipUrl: string): Promise<number> {
  ...
  try {
    ...
    console.log("[S3 Restore] Downloading ZIP from:", filesZipUrl);
    
    // Download ZIP file
    await downloadFile(filesZipUrl, zipPath);
    
    console.log("[S3 Restore] Extracting ZIP...");
    ...
    for (const entry of zipEntries) {
      if (!entry.isDirectory) {
        ...
        console.log(`[S3 Restore] Uploading: ${fileName}`);
    ...
    console.log(`[S3 Restore] Restored ${restoredCount} files`);
