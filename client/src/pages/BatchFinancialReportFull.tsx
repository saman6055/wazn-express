            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className={`flex justify-between items-center p-5 rounded-xl border ${
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className={`font-medium ${isProfitable ? 'text-blue-800 dark:text-blue-200' : 'text-red-800 dark:text-red-200'}`}>
                      {isProfitable
                ...
                <span className={`font-bold text-2xl ${isProfitable ? 'text-blue-700 dark:text-blue-300' : 'text-red-700 dark:text-red-300'}`}>
                  ${(() => {
                    const divisor = batch.shippingType === 'sea' ? financial.actualCbm : totalChargeableWeight;
                    return divisor > 0
            ...
            {financial.customerBreakdown?.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b bg-slate-100 dark:bg-slate-950/40">
                      <th className="text-right py-4 px-6 font-semibold text-slate-700 dark:text-slate-300">{pickLang(language, { ku: "کڕیار", en: "Customer", ar: "الزبون", zh: "客户" })}</th>
                      <th className="text-center py-4 px-4 font-semibold text-slate-700 dark:text-slate-300">{pickLang(language, { ku: "پاکەت", en: "Packages", ar: "الطرود", zh: "包裹" })}</th>
                      <th className="text-center py-4 px-4 font-semibold text-slate-700 dark:text-slate-300">
                        {batch.shippingType === 'sea' ? 'CBM' : pickLang(language, { ku: 'کێش حسابکراو', en: 'Chargeable weight', ar: 'الوزن المحتسب', zh: '计费重量' })}
                      </th>
                      <th className="text-center py-4 px-4 font-semibold text-slate-700 dark:text-slate-300">{pickLang(language, { ku: "داهات", en: "Revenue", ar: "الإيرادات", zh: "收入" })}</th>
                      <th className="text-center py-4 px-4 font-semibold text-slate-700 dark:text-slate-300">{pickLang(language, { ku: "ڕێژە", en: "Share", ar: "النسبة", zh: "占比" })}</th>
                      <th className="text-center py-4 px-4 font-semibold text-slate-700 dark:text-slate-300">{pickLang(language, { ku: "وردەکاری", en: "Details", ar: "التفاصيل", zh: "明细" })}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {financial.customerBreakdown
                        ...
                        >
                          <td className="py-4 px-6">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-medium text-slate-800 dark:text-slate-200">{getCustomerName(item.customerId)}</p>
                                <Badge variant="outline" className="text-xs mt-1">{getCustomerCode(item.customerId)}</Badge>
                              </div>
                            </div>
                          </td>
                          <td className="text-center py-4 px-4">
                            <Badge variant="secondary" className="bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 px-3 py-1">
                              {item.packages}
                            </Badge>
                          </td>
                  ...
                  <tfoot>
                    <tr className="bg-slate-100 dark:bg-slate-950/40 font-bold">
                      <td className="py-4 px-6 text-slate-700 dark:text-slate-300">{pickLang(language, { ku: "کۆی گشتی", en: "Grand total", ar: "المجموع الكلي", zh: "总计" })}</td>
                      <td className="text-center py-4 px-4">
        ...
        <Card className={`border-2 shadow-lg ${isProfitable 
          ...
          <CardContent className="flex items-center justify-between py-8 px-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className={`text-2xl font-bold ${isProfitable ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
                  {isProfitable
