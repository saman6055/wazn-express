            ...
            {product.status !== "out_of_stock" && (
              <div className="mt-8 rounded-3xl bg-white dark:bg-card border border-slate-200 dark:border-slate-800/60 p-5">
                ...
                <div className="space-y-3">
                  ...
                  <input value={form.customerCity} onChange={(e) => setForm({ ...form, customerCity: e.target.value })} placeholder={label({ ku: "شار", en: "City", ar: "المدينة", zh: "城市" })} className="w-full h-11 rounded-xl border border-slate-200 dark:border-slate-800/60 px-3.5 text-base md:text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-100 outline-none" />
                  <textarea value={form.customerAddress} onChange={(e) => setForm({ ...form, customerAddress: e.target.value })} rows={2} placeholder={label({ ku: "ناونیشانی گەیاندن", en: "Delivery address", ar: "عنوان التوصيل", zh: "配送地址" })} className="w-full rounded-xl border border-slate-200 dark:border-slate-800/60 px-3.5 py-2.5 text-base md:text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-100 outline-none resize-none" />
