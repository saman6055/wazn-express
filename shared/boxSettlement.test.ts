...
const parcel = (packageId: number, chargedUsd: number, extra: Partial<{ discountedUsd: number; settledUsd: number }> = {}) => ({
  packageId, chargedUsd, discountedUsd: 0, settledUsd: 0, ...extra,
});
...
describe("one parcel is set aside — the rest are still receipted", () => {
  it("leaves the held parcel off the total and keeps its money owed", () => {
    ...
    const t = settlementTotals(parcels, [{ packageId: 3, held: true }]);
    expect(t.dueUsd).toBe(20.40);
    expect(t.heldUsd, "held money must not vanish").toBe(7.50);
    const held = t.lines.find((l) => l.packageId === 3)!;
  ...
  it("refuses to discount a parcel nobody is paying for", () => {
    ...
    const t = settlementTotals([parcel(1, 10)], [{ packageId: 1, held: true, discountUsd: 4 }]);
    expect(t.discountUsd).toBe(0);
...
describe("a discount is money we chose to give", () => {
  it("comes off what is due and is counted as a discount", () => {
    const t = settlementTotals([parcel(1, 12)], [{ packageId: 1, discountUsd: 4 }]);
  ...
  it("cannot be negative, whatever is passed in", () => {
    const t = settlementTotals([parcel(1, 12)], [{ packageId: 1, discountUsd: -5 }]);
    expect(t.discountUsd).toBe(0);
  ...
  it("never turns a parcel into money owed back", () => {
    ...
    const t = settlementTotals([parcel(1, 12)], [{ packageId: 1, discountUsd: 30 }]);
...
describe("a correction moves the price, not the payment", () => {
  it("we overcharged, the customer noticed: the price comes down", () => {
    const t = settlementTotals([parcel(1, 6.25)], [{ packageId: 1, correctionUsd: -1.25 }]);
  ...
  it("we undercharged and they sent the difference: the price goes up", () => {
    const t = settlementTotals([parcel(1, 9)], [{ packageId: 1, correctionUsd: 2 }]);
  ...
  it("a correction past zero settles the parcel rather than owing it back", () => {
    const t = settlementTotals([parcel(1, 5)], [{ packageId: 1, correctionUsd: -20 }]);
    expect(t.dueUsd).toBe(0);
    expect(t.lines[0]!.remainingUsd).toBe(0);
  });
});
...
describe("the whole screen at once", () => {
  it("adds up the mockup's box exactly", () => {
    ...
    const t = settlementTotals(
      ...
      [
        { packageId: 2, discountUsd: 4.00 },
        { packageId: 3, correctionUsd: -1.25 },
        { packageId: 4, correctionUsd: 2.00 },
        { packageId: 5, held: true },
      ],
    );
...
describe("a discount on the whole box", () => {
  const box900 = [245.00, 180.50, 312.75, 96.25, 65.50].map((v, i) =>
    ({ packageId: i + 1, chargedUsd: v, discountedUsd: 0, settledUsd: 0 }));
  ...
  it("takes a lower rate per kilo: thirty kilos at eleven, call it ten", () => {
    const parcels = [
      { packageId: 1, chargedUsd: 220, weightKg: 20, discountedUsd: 0, settledUsd: 0 },
      { packageId: 2, chargedUsd: 110, weightKg: 10, discountedUsd: 0, settledUsd: 0 },
    ];
  ...
  it("refuses a rate that went up, which is a price change and not a gift", () => {
    const parcels = [{ packageId: 1, chargedUsd: 220, weightKg: 20, discountedUsd: 0, settledUsd: 0 }];
  ...
  it("does nothing when there is nothing left to discount", () => {
    const paid = [{ packageId: 1, chargedUsd: 50, discountedUsd: 0, settledUsd: 50 }];
  ...
  it("reaches the receipt as a lower total, which is what the customer pays", () => {
    ...
    const t = settlementTotals(
      box900,
      box900.map((p) => ({ packageId: p.packageId, discountUsd: shares.get(p.packageId) ?? 0 })),
    );
